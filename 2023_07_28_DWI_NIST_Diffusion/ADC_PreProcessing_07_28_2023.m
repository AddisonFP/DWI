...
addpath(genpath('/v/raid10/users/apowell/DWI/functions/'));
 % DWI and ADC processing functions !!!my copy has some differences from
 % Sara's original code, ie dcm select in createVialRois and custom b
 % values for getDWIsubset

% Define directories
datadir = '/v/raid10/users/apayne/2023/2023-07-28_DWI/20230728100700MR/DicomData/'; % the location of the .dcm files that start with 's0000## fl3d...'
workdir = '/v/raid10/users/apowell/DWI/2023_07_28_DWI_NIST_Diffusion/'; % where the proccessed plots are saved (should be the same folder as the one this script is in)
matdir = '/v/raid10/users/apowell/DWI/2023_07_28_DWI_NIST_Diffusion/DicomMats/'; % where the .mat dicom data is saved
cd(workdir);
...
% Phantom Position 1 
dicom_list = {{'s000003 fl3d_SEK05_dur=10_delay=200'; 's000004 fl3d_SEK05_dur=10_delay=200'};
              's000005 DWI2D_iPat3_RabbitProtocolCopy_TRACEW';};
...
dicom_dsc = {'SOS_Tri_d00_TR200_rot0'; 'EPI_rot0'};
    %'EPI_rot0'}; %this corresponds to the unknow TRACEW file


loadDWIdicoms_XA(datadir, matdir, 'REP1', dicom_list, dicom_dsc)

dicom_list = {{'s000010 fl3d_SEK05_dur=10_delay=200 run2'; 's000011 fl3d_SEK05_dur=10_delay=200 run2'};
              's000012 DWI2D_iPat3_RabbitProtocolCopy run2_TRACEW';};
loadDWIdicoms_XA(datadir, matdir, 'REP2', dicom_list, dicom_dsc)

%% Flatten the DICOMS
% use the B1 map to flatten each dicom
% dicom_lists = {dir([matdir 'REP1_EPI_*.mat']); dir([matdir 'REP2_EPI_*.mat']);
%    dir([matdir 'REP1_SOS_*.mat']); dir([matdir 'REP2_SOS_*.mat'])};
dicom_mat_list = {'REP1_SOS_Tri_d00_TR200_rot0'};
B1Flatten(dicom_mat_list, matdir, [datadir 's000016 tfl_b1map run2']);
...
[vials, bg_imge, bg_geomInfo] = createVialROIs(bg_imge_file, view_slice, radius, 'end');
